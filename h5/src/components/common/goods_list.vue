<template>
  ...
  <div class="content" v-if="goodsList && goodsList.length > 0">
    <div class="goods" v-for="(item,index) in goodsList" v-if="item.goodsObj.goodsStatus == 2" :key="index" @click="routeLink('detail',{'goods_id':item.goodsObj.goodsId})">
      <div class="pic">
        <soldout-mask w="120px" h="120px" v-if="item.goodsObj.isSalable == 0"></soldout-mask>
        <!-- <img :src="item.goodsObj.coverImage" @error="setErrorImg($event,120,120)"> -->
        <x-img container=".v-scroller" :src="item.goodsObj.coverImage + imageSize()" :default-src="defaultLazyImg()" :offset="150"></x-img>
      </div>
