          ...
          <div v-for="(item,index) in actTabList" :key="index">
            <div class="list-container">
              <div :class="{'active':son.isActive,'yure':son.actStatus == 1}" class="tab" v-for="(son, i) in item.group_acts" :key="i" @click="changeTab(son)">
                ...
                <p class="text" v-if="item.date_title == '昨日'">昨日开抢</p>
                <p class="text" v-else-if="item.date_title == '明日'">明日开抢</p>
        ...
        <div class="super" v-if="superSaleList && superSaleList.length > 0">
          ...
          <ul class="single-goods">
            ...
            <li class="goods-item" v-for="(item,index) in superSaleList" :key="index" @click="superToDetail(item)">
              <!-- 售馨 -->
              <!-- <soldout-mask w="100%" h="198px" v-if="item && item.isSalable == 0"></soldout-mask> -->
              <!-- <img class="goods-img" :src="item.coverImage" @error="setErrorImg($event,351,198)"> -->
              ...
              <div class="title">
                ...
                <!-- <span class="zhiyou" v-if="item.sellType == 2">赠品</span>
                <span class="zhiyou" v-if="item.sellType == 1">预售</span> -->
...
<script>
import flashSaleList from '@/components/common/flash-sale-list'
  ...
  methods: {
    getTabList() {
      ...
      this.$http.get('order/goods_promotion_tab.do').then(res => {
        ...
        // 倒计时功能
        // this.countDownLeft = this.selectedAct.activeEndTime - this.sysTime
        // this.countNum(this.countDownLeft / 1000)
      })
    },
    //判断是昨天、今天、明天
    isToday(topTab) {
    ...
    getActivityDetailList(pageNum) {
      ...
      this.$http.get('order/get_promotion_list.do', param).then(res => {
        let data = res.data
        ...
        if (data.promotionGoods && data.promotionGoods.length < 1) {
          this.isMore = 0
        } else {
          this.isMore = 1
        }
    ...
    loadMore() {
      if (!this.loadingFlag) {
        this.getActivityDetailList(this.pageNum)
      }
    },
    tabCenter() {
    ...
    scrollWidth() {
      if (this.$refs.headerList) {
        this.$refs.headerList.style.width =
          this.tabList.length * 80 / 37.5 + 'rem'
      }
      this.tabCenter()
    },
    changeTab(son) {
      ...
      // if (this.selectedAct.actStatus != 1) {
      //   //当前活动状态为抢购中或者是已开抢，需要倒计时
      //   if (son.activeEndTime - this.sysTime <= 0) {
      //     //活动已结束
      //     // this.count_empty = true
      //   } else {
      //     // this.countNum((son.activeEndTime - this.sysTime) / 1000)
      //   }
      // }
      this.getActivityDetailList()
    }
    ...
    // countNum(time) {
    //   let self = this
    //   self.timerData = new Date(time).formatStamp()
    //   if (self.countNumTimer) {
    //     //如果有计时器，先清除计时器
    //     clearInterval(self.countNumTimer)
    //   }
    //   self.countNumTimer = setInterval(function() {
    //     time = time - 1
    //     self.timerData = new Date(time).formatStamp()
    //     if (time <= 0) {
    //       clearInterval(self.countNumTimer)
    //       // self.count_empty = true //倒计时结束
    //       self.selectedAct.actStatus = 4
    //       //手动更改tab栏的文案
    //       let oActive = document
    //         .getElementsByClassName('active')[0]
    //         .getElementsByClassName('text')[0]
    //       oActive.innerHTML = '已结束'
    //     }
    //   }, 1000)
    // }
  }
